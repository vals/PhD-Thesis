...
This chapter takes the concepts introduced in Chapter \ref{ch:zebrafish} and rework them in a spatial context, rather than temporal. Here we also formulize the statistics of the significance test, and provide computational speedups. The analysis in Chapters \ref{ch:zebrafish} and \ref{ch:malaria} had to be run on hundreds of compute nodes to finish, with the work presented in this chapter the analysis were reproduced in just a few minutes on standard desktop computer.

First we present the motivation and analysis results on public data using our method. Following this, we go through the model in detail.
